...
export async function GET(req: NextRequest) {
  ...
  if (!session?.user) return NextResponse.json({ users: [], pages: [] }, { status: 401 });
  const userId = (session.user as { id: string }).id;

  const slug = req.nextUrl.searchParams.get("ws");
  const q = (req.nextUrl.searchParams.get("q") ?? "").trim();
  if (!slug) return NextResponse.json({ users: [], pages: [] });
  ...
  if (!ws || ws.members.length === 0) {
    return NextResponse.json({ users: [], pages: [] }, { status: 403 });
  }
